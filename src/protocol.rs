...
pub const CAPABILITY_OPEN_METADATA: u64 = 1 << 0;
pub const CAPABILITY_PRIORITY_HINTS: u64 = 1 << 1;
pub const CAPABILITY_STREAM_GROUPS: u64 = 1 << 2;
pub const CAPABILITY_PRIORITY_UPDATE: u64 = 1 << 3;
pub const DEFAULT_CAPABILITIES: u64 = CAPABILITY_OPEN_METADATA
...
pub const SETTING_MAX_CONTROL_PAYLOAD_BYTES: u64 = 8;
pub const SETTING_MAX_EXTENSION_PAYLOAD_BYTES: u64 = 9;
pub const SETTING_SCHEDULER_HINTS: u64 = 10;
pub const SETTING_PING_PADDING_KEY: u64 = 11;
pub const SETTING_PREFACE_PADDING: u64 = 12;
...
mod tests {
    ...
    fn registry_values_match_cross_language_contract() {
        ...
        assert_eq!(CAPABILITY_OPEN_METADATA, 1);
        assert_eq!(CAPABILITY_PRIORITY_HINTS, 1 << 1);
        assert_eq!(CAPABILITY_STREAM_GROUPS, 1 << 2);
        assert_eq!(CAPABILITY_PRIORITY_UPDATE, 1 << 3);
        assert_eq!(DEFAULT_CAPABILITIES, 15);

        assert_eq!(EXT_PRIORITY_UPDATE, 1);

        assert_eq!(SETTING_MAX_CONTROL_PAYLOAD_BYTES, 8);
        assert_eq!(SETTING_MAX_EXTENSION_PAYLOAD_BYTES, 9);
        assert_eq!(SETTING_SCHEDULER_HINTS, 10);
